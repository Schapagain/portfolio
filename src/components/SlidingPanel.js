...
export default function SlidingPanel({ hiddenContent, mainContent }) {
  ...
  const websiteClass = classNames(
    "flex-col flex justify-end z-10 lg:absolute w-full h-1/2 lg:h-full -left-0 top-0",
    {}
  );
  const descriptionClass = classNames(
    "flex text-sm md:text-md lg:text-lg h-1/2 lg:h-full right-0 lg:absolute w-full lg:w-2/3"
  );

  return (
    <div className="lg:relative z-30 flex-col lg:flex h-full max-h-104 w-full md:w-2/3 m-auto">
      <animated.div style={mainContentProps} className={websiteClass}>
        <div className="flex h-1/2 h-full w-full">{mainContent}</div>
        <div
